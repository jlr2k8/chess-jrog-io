#!/usr/bin/env bash
# Deploy CodePipeline + CodeBuild for chess.jrog.io (GitHub main → ECR → App Runner auto-deploy).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
REGION="${AWS_REGION:-us-west-2}"
PIPELINE_STACK="${PIPELINE_STACK_NAME:-chess-jrog-io-pipeline}"
APP_STACK="${APP_STACK_NAME:-chess-jrog-io}"
...
GITHUB_REPO="${GITHUB_REPO:-chess-jrog-io}"
...
DOMAIN="${DOMAIN_NAME:-chess.jrog.io}"
...
echo ""
echo "Create https://github.com/$GITHUB_OWNER/$GITHUB_REPO and push main, then the pipeline deploys."
